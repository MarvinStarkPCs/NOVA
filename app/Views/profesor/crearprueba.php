...
function agregarBloque() {
  ...
  if (!asignatura) {
    Swal.fire({ icon: "warning", title: "Selecciona una asignatura", confirmButtonColor: "#B3C69B" });
    return;
  }
...
function reiniciarFormulario() {
  Swal.fire({
    ...
    showCancelButton: true, confirmButtonColor: '#B3C69B', cancelButtonColor: '#d33', confirmButtonText: 'Sí, reiniciar'
  }).then(result => {
...
function guardarPrueba() {
  ...
  if (!nombrePrueba || !descripcionPrueba) {
    Swal.fire({ icon: 'error', title: 'Datos incompletos', text: 'Completa el nombre y descripción de la prueba.', confirmButtonColor: '#B3C69B' });
    return;
  }

  const bloques = document.querySelectorAll(".bloque");
  if (bloques.length === 0) {
    Swal.fire({ icon: "warning", title: "Agrega al menos un bloque", confirmButtonColor: "#B3C69B" });
    return;
  }
  ...
  bloques.forEach((bloque, i) => {
    ...
    if (tipo === 'con-texto') {
      ...
      if (!titulo || !texto) {
        Swal.fire({ icon: 'error', title: `Bloque ${i + 1}`, text: 'Completa el título y texto de la lectura.', confirmButtonColor: '#B3C69B' });
        error = true; return;
      }
    }
    const preguntas = bloque.querySelectorAll(".pregunta");
    if (preguntas.length === 0) {
      Swal.fire({ icon: 'error', title: `Bloque ${i + 1}`, text: 'El bloque debe tener al menos una pregunta.', confirmButtonColor: '#B3C69B' });
      error = true; return;
    }
    const preguntasData = [];
    preguntas.forEach((preguntaElem, j) => {
      ...
      if (!enunciado || opciones.some(opt => !opt) || !correcta || !justificacion) {
        Swal.fire({ icon: 'error', title: `Bloque ${i + 1} - Pregunta ${j + 1}`, text: 'Completa todos los campos y selecciona la respuesta correcta.', confirmButtonColor: '#B3C69B' });
        error = true; return;
      }
