...
function agregarBloque() {
  ...
  if (!asignatura) {
    Swal.fire({ icon: "warning", title: "Selecciona una asignatura", confirmButtonColor: "#2A6322" });
    return;
  }
...
function reiniciarFormulario() {
  Swal.fire({
    ...
    showCancelButton: true, confirmButtonColor: '#2A6322', cancelButtonColor: '#d33', confirmButtonText: 'Sí, reiniciar'
  }).then(result => {
...
function guardarPrueba() {
  ...
  if (!nombrePrueba || !descripcionPrueba) {
    Swal.fire({ icon: 'error', title: 'Datos incompletos', text: 'Completa el nombre y descripción de la prueba.', confirmButtonColor: '#2A6322' });
    return;
  }

  const bloques = document.querySelectorAll(".bloque");
  if (bloques.length === 0) {
    Swal.fire({ icon: "warning", title: "Agrega al menos un bloque", confirmButtonColor: "#2A6322" });
    return;
  }
  ...
  bloques.forEach((bloque, i) => {
    ...
    if (tipo === 'con-texto') {
      ...
      if (!titulo || !texto) {
        Swal.fire({ icon: 'error', title: `Bloque ${i + 1}`, text: 'Completa el título y texto de la lectura.', confirmButtonColor: '#2A6322' });
        error = true; return;
      }
    }
    const preguntas = bloque.querySelectorAll(".pregunta");
    if (preguntas.length === 0) {
      Swal.fire({ icon: 'error', title: `Bloque ${i + 1}`, text: 'El bloque debe tener al menos una pregunta.', confirmButtonColor: '#2A6322' });
      error = true; return;
    }
    const preguntasData = [];
    preguntas.forEach((preguntaElem, j) => {
      ...
      if (!enunciado || opciones.some(opt => !opt) || !correcta || !justificacion) {
        Swal.fire({ icon: 'error', title: `Bloque ${i + 1} - Pregunta ${j + 1}`, text: 'Completa todos los campos y selecciona la respuesta correcta.', confirmButtonColor: '#2A6322' });
        error = true; return;
      }
